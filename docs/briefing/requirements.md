...
| FR-C-08b | **シード一覧は分野を横断していること。** 供給層を人ごとに削らない（FR-C-08）以上、偏りはランキングでは取り消せず、一覧そのものでしか直せない。特定個人のライブラリから逆算した一覧をそのまま全員の初期値にしない。**登録時に分野を選ばせる形は採らない** ── 選ばなかった分野が入らないのは供給層の間引きそのもので、興味が変わっても届かなくなる。`profileIndependent` の件数は取り込み上限 × `PROFILE_INDEPENDENT_FLOOR` の候補数を実際に賄えるだけ確保する（ソース数ではなく候補数が床であることに注意）
...
| FR-U-13 | 訳題と日本語要約は **rank で表示が確定した候補にだけ**生成する（1 日 `briefingSize` 件）。生成は rank とは別ジョブで、Briefing の生成をブロックしない。結果は `recCandidates` に永続キャッシュし、同じ記事が翌日も選ばれたら再生成しない。プロバイダはローカル（Ollama）と外部を env で切り替える。RSS の抜粋が短い候補は本文を取得して要約の入力にする。外部プロバイダでは複数件を 1 回の呼び出しでまとめて生成でき（`RECOMMENDER_DIGEST_BATCH_SIZE`）、応答から読めなかった候補だけを単発で作り直す |
